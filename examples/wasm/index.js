...
async function fetchFragmentWithRangeRequests(url, schema, columnTypes, limit, fileSize) {
    ...
    for (let i = 0; i < schema.length && i < numColumns; i++) {
        ...
        const columnRows = Math.min(length, limit);
        ...
        try {
            if (type === 'string' && bufferOffsets.length >= 2) {
                ...
                const offsetsToFetch = columnRows * bytesPerOffset;
                ...
                const maxStringEnd = stringEnds[columnRows - 1] || 0;
                ...
                const floatArr = new Float64Array(columnRows);
                for (let j = 0; j < columnRows; j++) floatArr[j] = f32Arr[j];
                columns[name] = floatArr;
                ...
                const dim = type.dim;
                const bufferStart = bufferOffsets[0];
                const totalFloats = columnRows * dim;
                const bytesToFetch = totalFloats * 4;
                console.log(`[Worker] Range: Reading vector ${name}: dim=${dim}, rows=${columnRows}, bytes=${bytesToFetch}`);
                ...
                columns[name] = new Float64Array(columnRows).fill(0);
            }
        } catch (e) {
    ...
    let minVectorRows = Infinity;
    for (const [name, data] of Object.entries(columns)) {
        if (name.startsWith('__')) continue;
        const dim = columns[`__${name}_dim`];
        let rows = 0;
        if (data instanceof Float32Array && dim && dim > 0) {
            rows = Math.floor(data.length / dim);
            minVectorRows = Math.min(minVectorRows, rows);
    ...
    let actualRows = minRows === Infinity ? 0 : minRows;
    if (minVectorRows !== Infinity) {
        actualRows = Math.min(actualRows, minVectorRows);
    }

    const expectedRows = Math.min(totalRows, limit);
    if (actualRows !== expectedRows) {
        console.log(`[Worker] Range: Normalized row count from ${expectedRows} to ${actualRows} to align column lengths`);
    }
    ...
    for (const [name, data] of Object.entries(columns)) {
        ...
        const dim = columns[`__${name}_dim`];
        if (data instanceof Float32Array && dim && dim > 0) {
            const maxLen = actualRows * dim;
            if (data.length > maxLen) columns[name] = data.slice(0, maxLen);
        } else if (Array.isArray(data)) {
            if (data.length > actualRows) columns[name] = data.slice(0, actualRows);
        } else if (ArrayBuffer.isView(data)) {
            if (data.length > actualRows) columns[name] = data.slice(0, actualRows);
        }
    }
